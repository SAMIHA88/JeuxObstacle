...
class Vehicle {
  ...
  evadeLeaderZone(leaderPos, leaderHeading, zoneWidth) {
    ...
    if (distanceToEvadePoint < halfZoneWidth + this.radius) {
...
applyBehaviors(target, obstacles, vehicles, separationDistance) {
  ...
  // Nouvelle méthode pour l'évasion de la zone devant le leader
  if (this.isFollower) {
    this.evadeLeaderZone(vehicles[0].pos, vehicles[0].vel.copy().normalize(), leaderEvadeZoneWidth);
  }
}
...
drawEvadeZone(evadePoint, zoneWidth) {
  // Dessiner une ligne grise devant le véhicule pour représenter la zone d'évasion
  stroke(150);
  strokeWeight(2);
  line(this.position.x, this.position.y, evadePoint.x, evadePoint.y);
}
evadeLeaderZone(leaderPos, leaderHeading, zoneWidth) {
  let evadePoint = this.calculateEvadePoint(leaderPos, leaderHeading, zoneWidth);
  this.drawEvadeZone(evadePoint, zoneWidth);
  ...
  // Calculer la distance entre le véhicule et le point d'évasion
  let distanceToEvadePoint = this.position.dist(evadePoint);
